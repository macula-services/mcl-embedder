...
The service boots, joins the mesh and answers `/health` on 8480. It
...
a different libc.
    ...
    podman build -t mcl-embedder -f Containerfile .
...
| `MCL_HEALTH_PORT` | `8480` | Health endpoint. Host networking makes a collision a silent bind failure, so check the host before changing.  |
| `MCL_NODE_NAME` | `mcl_embedder` | Erlang node name. |
| `MCL_NODE_HOST` | `127.0.0.1` | Erlang node host. |
| `MCL_COOKIE` | `mcl_embedder` | Erlang cookie. |
...
`ghcr.io/macula-services/mcl-embedder:latest` plus the semver tag. Pull `:latest` under
...
Six callbacks in `mcl_embedder_service`, all required, all resolved **by name** by
