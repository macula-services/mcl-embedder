...
%%
%% On macula 12, serving an org-namespaced procedure (`mcl-embedder/embed')
...
%% the answer for mcl_embedder_service:health/0. It changes nothing: advertising
%% stays mcl_om's job.
...
procedures() ->
    ...
    [mcl_om_capabilities:org_procedure(Org, Name)
     || #{name := Name} <- mcl_embedder_service:capabilities()].
